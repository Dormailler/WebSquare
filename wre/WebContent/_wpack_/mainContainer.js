...
scwin.dispPage = async function (argMenuID, argSelectedTab) {
    ...
    if (!isFirstOpen || (isFirstOpen && argSelectedTab !== false)) {
        let numTabIndex = tac_main.getTabIndex(strTabID);

        if (numTabIndex == 0 || numTabIndex) {
            tac_main.setSelectedTabIndex(numTabIndex);
        } else {
            // 엔진 오류 - 5.0_5.4991B.20230824.141458_1.8 : tabIndex is null
            $p.setTimeout(function (argTabID) {
                tac_main.setSelectedTabIndex(tac_main.getTabIndex(argTabID));
            }, {
                delay: 400,
                args: [strTabID]
            });
